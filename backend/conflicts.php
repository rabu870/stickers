...
if ($_GET['func'] == 'load') {
    $students = [];
    $keys = $db->query('SELECT `id` FROM `students` WHERE true')->fetch_all();
    foreach ($keys as $id) {
        array_push($students, [$id[0], $db->query("SELECT `class_id` FROM `stickers` WHERE `student_id` = " . $id[0])->fetch_all()]);
    }
